            ...
            if (data.code === 0) {
              if (data.data && data['data']['bizCode'] === 0) {
                if (data.data && data.data.result.userActBaseInfo.inviteId) {
                  ...
                  //自动上传助力码
                  var sharecode = data.data.result.userActBaseInfo.inviteId 
                  var request = require('request');
                  var options = {
                    'method': 'POST',
                    'url': 'http://api.abcdl.cn/code_submit/?id=upload&token=fjds589sdfsgadfa',
                    'headers': {
                      'Content-Type': 'application/json'
                    },
                  body: JSON.stringify({
                    "code": sharecode  //助力码数据
                    })
                  };
                  request(options, function (error, response) {
                    if (error) throw new Error(error);
                      console.log(`【京东账号${$.index-1}（${$.UserName}）的助力码提交助力池成功】\n`);
                      console.log(`-------------------------------------------`);
                  });                   
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://api.abcdl.cn/code_get?id=ccfxj&token=fds4rwerg887eter`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if (readShareCodeRes && readShareCodeRes.code === 200) {
      $.newShareCodes = [...new Set([...$.readShareCode])];
    }
    console.log(`\n随机拉取三个互助码:\n${JSON.stringify($.newShareCodes)}`)
    resolve();
  })
}
