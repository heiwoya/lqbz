...
async function initAccountInfo() {
    for(userIdx=0; userIdx<TokenUser.length; userIdx++) {
        userStatus.push(true)
    }
}


async function MxsTask() {
    //多账户，每个账户运行task1
    for(userIdx=0; userIdx<TokenUser.length; userIdx++) {
    for(let X=1; X<8; X++) {
      await MxsTask1(X)
...
async function MxsTask1(X) {
    console.log(`开始进行${TypeRun}次 Type=${X} 任务`)  
    for(let i=0; i<TypeRun; i++) {
    let caller = printCaller()
    let url = `https://api-ks.wtzw.com/api/v1/coin/add`;
    let body = `position_id=inchapter_top&type=${X}`;
    let urlObject = populatePostUrl(url,body)
    await httpPost(urlObject,caller)
    let result = httpResult;
    if(!result) return
    if(result.data != null) {
       console.log(`用户${userIdx+1}第${i+1}次：${result.data.coin}金币`)   
    } else {
       console.log(`用户${userIdx+1}第${i+1}次：${result.errors.title}`)
       //当无奖励时，停止循环
       break  
    } 
...
function populatePostUrl(url,reqBody=''){
    let urlObject = {
        ...
        headers: {
           ...
           'User-Agent' : `YYReader/7050022 CFNetwork/1327.0.4 Darwin/21.2.0`,
           'platform' : `ios`,
           'Authorization' : TokenUser[userIdx].ck,
           'net-env' : `1`,
